...
func (d *DockerDriver) Fingerprint(cfg *config.Config, node *structs.Node) (map[string]string, error) {
	...
	nodeAttributes[dockerDriverAttr] = "1"
	nodeAttributes["driver.docker.version"] = env.Get("Version")

	privileged := d.config.ReadBoolDefault(dockerPrivilegedConfigOption, false)
	if privileged {
		nodeAttributes[dockerPrivilegedConfigOption] = "1"
	}
	...
	if d.config.ReadBoolDefault(dockerVolumesConfigOption, dockerVolumesConfigDefault) {
		nodeAttributes["driver."+dockerVolumesConfigOption] = "1"
	}
	...
	if nets, err := client.ListNetworks(); err != nil {
		...
		for _, n := range nets {
			...
			if n.IPAM.Config[0].Gateway != "" {
				nodeAttributes["driver.docker.bridge_ip"] = n.IPAM.Config[0].Gateway
